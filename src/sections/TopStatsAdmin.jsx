...
const TopStatsAdmin = () => {
  return (
    <section className="w-full py-14 bg-linear-to-br from-zinc-950 via-zinc-900 to-black">
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat) => (
            <div
              key={stat.id}
              className="relative overflow-hidden rounded-3xl p-6 bg-white/5 backdrop-blur-xl border border-white/10 shadow-xl hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group"
            >
              {/* Glow Effect */}
              ...
              {/* Title */}
              <p className="text-sm text-zinc-400 relative z-10 tracking-wide">
                {stat.title}
              </p>
            </div>
          ))}
